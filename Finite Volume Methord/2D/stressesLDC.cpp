...
int x = 41; // number of grid points in radial direction (across diameter)
int y = 41; // number of grid points in film thickness

const double lx = 1.0; // diameter of circular disc (in m)
const double ly = 1.0; // film thickness (in m)
...
const double temperature = 300; // assuming isothermal process
...
void solveStresses(vector<vector<double>>& tau_xx, vector<vector<double>>& tau_yy, vector<vector<double>>& tau_xy, const vector<vector<double>>& u, const vector<vector<double>>& v) {
    // normal stresses
    for (int i = 1; i < x; i++) {
        for (int j = 1; j < y; j++) {
            ...
            tau_xx[i][j] = 2.0 * mu * dudx - (2.0/3.0) * mu * (dudx + dvdy);
            tau_yy[i][j] = 2.0 * mu * dvdy - (2.0/3.0) * mu * (dudx + dvdy);
        }
    }

    for (int j = 0; j <= y; j++) {
        tau_xx[0][j] = tau_xx[1][j];
        tau_xx[x][j] = tau_xx[x-1][j];
        
        tau_yy[0][j] = tau_yy[1][j];
        tau_yy[x][j] = tau_yy[x-1][j];
    }
    // Top and Bottom walls
    for (int i = 0; i <= x; i++) {
        tau_xx[i][0] = tau_xx[i][1];
        tau_xx[i][y] = tau_xx[i][y-1];
        
        tau_yy[i][0] = tau_yy[i][1];
        tau_yy[i][y] = tau_yy[i][y-1];
    }

    // shear stress
    for (int i = 0; i < x; i++) {
        for (int j = 0; j < y; j++) {
            double dudy = (u[i][j+1] - u[i][j]) / dy;
            double dvdx = (v[i+1][j] - v[i][j]) / dx;
...
void solveMomentum(vector<vector<double>>& u, vector<vector<double>>& v, const vector<vector<double>>& p, const vector<vector<double>>& rho, vector<vector<double>>& u_star, vector<vector<double>>& v_star, vector<vector<double>>& ap_u, vector<vector<double>>& ap_v, const vector<vector<double>>& u_old, const vector<vector<double>>& v_old, const vector<vector<double>>& rho_old, vector<vector<double>>& tau_xx, vector<vector<double>>& tau_yy, vector<vector<double>>& tau_xy) {
    ...
    for (int i = 1; i < x-1; i++) {
        for (int j = 1; j < y; j++) {
            ...
            // hybrid scheme
            double aw = max(Fw, 0.0);
            double ae = max(-Fe, 0.0);
            double as = max(Fs, 0.0);
            double an = max(-Fn, 0.0);
            ...
            double normal_stress = (tau_xx[i+1][j] - tau_xx[i][j]) * dy;
            double shear_stress = (tau_xy[i][j] - tau_xy[i][j-1]) * dx;
            U.b[i][j] = pres_u + normal_stress + shear_stress + ap0 * u_old[i][j] + (1.0 - alpha_u) * U.aP[i][j] * u[i][j];
            ap_u[i][j] = U.aP[i][j];
        }
    }
    ...
    for (int i = 1; i < x; i++) {
        for (int j = 1; j < y-1; j++) {
            ...
            double normal_stress = (tau_yy[i][j+1] - tau_yy[i][j]) * dx;
            double shear_stress = (tau_xy[i][j] - tau_xy[i-1][j]) * dy;
            V.b[i][j] = pres_v + normal_stress + shear_stress +  ap0 * v_old[i][j] + (1.0 - alpha_v) * V.aP[i][j] * v[i][j];
            ap_v[i][j] = V.aP[i][j];
        }
...
void writeResults(const vector<vector<double>>& u, const vector<vector<double>>& v, const vector<vector<double>>& p, const vector<vector<double>>& rho, const vector<vector<double>>& tau_xx, const vector<vector<double>>& tau_xy, const vector<vector<double>>& tau_yy, int iter=1) {
    ...
    // Write CSV header
    outfile << "x,y,X,Y,Z,U,V,P,rho,tau_xx,tau_xy,tau_yy\n";
    ...
    for (int j = 0; j < y; ++j) {
        for (int i = 0; i < x; ++i) {
            ...
            double tau_xx_center = 0.25 * (tau_xx[i][j] + tau_xx[i + 1][j] + tau_xx[i][j + 1] + tau_xx[i + 1][j + 1]);
            double tau_xy_center = tau_xy[i][j];
            double tau_yy_center = 0.25 * (tau_yy[i][j] + tau_yy[i + 1][j] + tau_yy[i][j + 1] + tau_yy[i + 1][j + 1]);

            outfile << i + 1 << "," << j + 1 << "," << x_pos << "," << y_pos << "," << 0.0 << "," << u_center << "," << v_center << "," << p_center << "," << rho_center << "," << tau_xx_center << "," << tau_xy_center << "," << tau_yy_center << "\n";
        }
    }
...
int main() {
    ...
    // stresses
    vector<vector<double>> tau_xx(x+1, vector<double>(y+1, 0.0));
    vector<vector<double>> tau_yy(x+1, vector<double>(y+1, 0.0));
    vector<vector<double>> tau_xy(x, vector<double>(y, 0.0));
    ...
    for (double time = 0; time < T; time += dt) {
        ...
        for (int iter = 0; iter < t_loop; iter++) {
            ...
            solveStresses(tau_xx, tau_yy, tau_xy, u, v);
            
            solveMomentum(u, v, p, rho, u_star, v_star, ap_u, ap_v, u_old, v_old, rho_old, tau_xx, tau_yy, tau_xy);
            ...
            for (int j = 0; j <= y; j++) {
                max_err = max(max_err, abs(u_history[21][j] - u[21][j]));
            }
            
            cout << "Iteration: " << iter << " " << max_err << '\r' << flush;
        }
        writeResults(u, v, p, rho, tau_xx, tau_xy, tau_yy, time);
    }
