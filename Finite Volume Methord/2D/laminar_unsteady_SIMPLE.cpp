...
void solveMomentum(const vector<vector<double>>& u, const vector<vector<double>>& v, vector<vector<double>>& u_star, vector<vector<double>>& v_star, vector<vector<double>>& ap_u, vector<vector<double>>& ap_v, const vector<vector<double>>& p, const vector<vector<double>>& u_old, const vector<vector<double>>& v_old) {
    ...
    for (int i = 1; i < x - 1; i++) {
        for (int j = 1; j < y; j++) {
            ...
            if (abs(ap_u_unrelaxed) > 1e-9) {
                ...
                double source = ae * u[i + 1][j] + aw * u[i - 1][j] + an * u[i][j + 1] + as * u[i][j - 1] + pres_u + ap0 * u_old[i][j] + (1.0 - alpha_u) * ap_u[i][j] * u[i][j];
    ...
    for (int i = 1; i < x; i++) {
        for (int j = 1; j < y - 1; j++) {
            ...
            if (abs(ap_v_unrelaxed) > 1e-9) {
                 ...
                 double source = ae * v[i + 1][j] + aw * v[i - 1][j] + an * v[i][j + 1] + as * v[i][j - 1] + pres_v + ap0 * v_old[i][j] + (1.0 - alpha_v) * ap_v[i][j] * v[i][j];
...
int main() {
    ...
    for (double time = 0; time < T; time += dt) {
        ...
        for (int iter = 0; iter < t_loop; iter++) {
            ...
            solveMomentum(u, v, u_star, v_star, ap_u, ap_v, p, u_old, v_old);
